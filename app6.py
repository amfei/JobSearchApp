# =====================================================
# 🧭 AI-Powered Job Search Ranker (Final Minimal Update - No LangGraph)
# =====================================================
import os
...
def get_sidebar_inputs():
    with st.sidebar:
        ...
        # 🔥 NEW — Exclude companies
        excluded_companies = [
            c.strip()
            for c in st.text_area(
                "Exclude companies containing (comma-separated):",
                ", ".join(EXCLUDED_COMPANIES)
                if "EXCLUDED_COMPANIES" in globals()
                else "",
            ).split(",")
            if c.strip()
        ]
...
def upload_cv():
    st.markdown(
        '<div class="section-title">📄 Upload Your CV</div>', unsafe_allow_html=True
    )
...
def fetch_and_store_jobs(
    ...
    with st.expander("🔍 Job Scan & Store", expanded=is_open):
        ...
        if scrape_now:
            ...
            with st.spinner(
                " 🧹 Clearing / scraping / embedding / storing…(titles, location, descriptions, filters ..)"
            ):
                jobs = scrape_linkedin_jobs(
                    job_titles, locations, num_jobs, days_filter, excluded_titles
                )
...
def extract_contact_info(cv_text):
    ...
    m = re.search(
        r"(\+?\d{1,3}[\s-]?)?(\(?\d{3}\)?[\s-]?\d{3}[\s-]?\d{4})", cv_text
    )
    phone = m.group(0) if m else ""
    m = re.search(r"(https?://)?(www\.)?linkedin\.com/[^\s]+", cv_text)
    linkedin = m.group(0) if m else ""
...
def rank_and_display_jobs(
    ...
    st.session_state["rank_log_time"] = datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    if generate_cover_opt:
        ...
        for i, (cos, job, hybrid, bm25) in enumerate(parsed[:top_n], 1):
            ...
            cover_letters.append(
                {"title": title, "company": company, "cover_letter": letter}
            )
...
def show_persistent_ranked_jobs():
    ...
    with st.expander(
        f"📋 Top Matched Jobs (last updated: {run_time})", expanded=is_open
    ):
        ...
        if not ranked_jobs:
            st.info(
                "ℹ️ No ranked jobs available yet. Press **Rank Matches** to generate them."
            )
            return
            ...
            if cover_letters:
                match = next(
                    (
                        ...
                        if cl["title"] == title and cl["company"] == company
                    ),
                    None,
                )
                if match:
...
def show_persistent_evaluation():
    ...
    with st.expander(
        f"📊 Evaluation Metrics (last updated: {run_time})", expanded=is_open
    ):
...
def show_persistent_visualization():
    ...
    with st.expander(
        f"🌐 Embedding Visualization (last updated: {run_time})", expanded=is_open
    ):
        st.markdown("### 🌐 Embedding Plot")

        if fig is None:
            st.info(
                "ℹ️ No visualization available yet. Generate it by pressing **📊 Visualize Embeddings**."
            )
            return
...
def main():
    ...
    # ========================================
    # 🧠 Ranking Logging (if any already exist)
    # ========================================
    ranked = st.session_state.get("ranked_jobs", [])

    if ranked:
        first_job = ranked[0][1]
    ...
    # ========================================
    # 🧮 Evaluation (Direct, No LangGraph)
    # ========================================
    if evaluate_now:
        ranked_jobs = st.session_state.get("ranked_jobs", [])
        if not cv_text.strip():
            ...
            st.warning("⚠️ No ranked jobs found. Please run ranking first.")
        else:
            with st.spinner("Evaluating ranking performance..."):
                try:
                    ...
                    if metrics:
                        ...
                        st.session_state["eval_log_time"] = datetime.now().strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                    else:
                        st.warning("⚠️ No evaluation metrics computed.")
                except Exception as e:
    ...
    if visualize_now:
        ...
        if not cv_text.strip():
            ...
            try:
                ...
                st.session_state["viz_log_time"] = datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
    ...
    if send_email_opt:
        ...
        if ranked_jobs:
            ...
            for i, item in enumerate(ranked_jobs, 1):
                ...
                lines.append(
                    f"{i}. {title} at {company} ({location}) — Hybrid {hybrid:.3f}"
                )
                lines.append(f"   {link}")
            ...
            try:
                send_email(
                    recipient_email, "Daily AI-Powered Job Matches", email_body
                )
            ...
            st.warning(
                "⚠️ No ranked jobs to email. Please run ranking first."
            )
